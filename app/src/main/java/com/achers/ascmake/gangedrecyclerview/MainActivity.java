...
public class MainActivity extends AppCompatActivity implements CheckListener {
    ...
    private SortDetailFragment2 mSortDetailFragment2;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_ganged_main);
        mContext = this;
    }
...
//        fragmentTransaction.commit();
        mSortDetailFragment2 = new SortDetailFragment2();
        Bundle bundle = new Bundle();
        bundle.putParcelableArrayList("right", mSortBean.getCategoryOneArray());
        mSortDetailFragment2.setArguments(bundle);
        mSortDetailFragment2.setListener(this);
        fragmentTransaction.add(R.id.lin_fragment, mSortDetailFragment2);
        fragmentTransaction.commit();
    }
...
//            mSortDetailFragment.setData(count);
            if(count==0){
                mSortDetailFragment2.setData(count);

            }else {
                mSortDetailFragment2.setData(count+1);

            }
//            ItemHeaderDecoration.setCurrentTag(String.valueOf(targetPosition));//凡是点击左边，将左边点击的位置作为当前的tag
            ItemHeaderDecoration2.setCurrentTag(String.valueOf(targetPosition));//凡是点击左边，将左边点击的位置作为当前的tag
        } else {
            if (isMoved) {
                isMoved = false;
            } else
                mSortAdapter.setCheckedPosition(position);
            ItemHeaderDecoration2.setCurrentTag(String.valueOf(position));//如果是滑动右边联动左边，则按照右边传过来的位置作为tag
