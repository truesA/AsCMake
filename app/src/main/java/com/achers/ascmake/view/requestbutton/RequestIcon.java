...
public class RequestIcon extends View {
    ...
    protected void onDraw(Canvas canvas) {
        super.onDraw(canvas);
        Log.e("iconStyle",iconStyle+"");
        switch (state) {
            case IDLE:
                break;
            case PROGRESS:
                float startAngle = 359f * secondProgress * speedMultiplier / CIRCLE_PROGRESS_MAX;
                float sweepAngle = 300f * speedMultiplier * (firstProgress - secondProgress) / CIRCLE_PROGRESS_MAX;
                if (startAngle > 360f)
                    startAngle = 360f;
                if (sweepAngle < 0f)
                    sweepAngle = 0f;
