package com.achers.ascmake.view.customstatusview;
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (mStatus == StatusEnum.Loading) {//正在加载
            ...
            if (circleValue == 1) {//表示圆画完了,可以钩了
                ...
                successPath.lineTo(getWidth() / 2-9, getWidth() / 5 * 3-4);
                ...
                Log.e("successPath",successValue+"");
            }
            Log.e("successPath下一个p",successValue+"");
        } else {//加载失败
            ...
            if (failValueRight == 1) {    //表示叉叉的右边部分画完了,可以画叉叉的左边部分
                ...
                canvas.drawPath(mPathCircleDst, mPaint);
                Log.e("failurePathLeft",failValueLeft+"");
            }
            Log.e("failurePathLeft下一个p",failValueLeft+"");
        }
    }
