package com.achers.ascmake.arout.slide.widget;
...
public class SlideBackIconView extends View {
    ...
    public void updateSlideLengthUp(Boolean overBack) {
        if (overBack){
            updateSlideLength(0);
            return;
        }
        ValueAnimator valueAnimator = ValueAnimator.ofFloat(slideLength, 0);
        valueAnimator.setDuration(200);
        valueAnimator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
            @Override
            public void onAnimationUpdate(ValueAnimator animation) {
                float currentValue = (float) animation.getAnimatedValue();
                updateSlideLength(currentValue);
            }
        });
